...
public:
	...
	inline SFPacket& operator << (unsigned char Data)
	{
	...
	inline SFPacket& operator << (unsigned short Data)
	{
	...
	inline SFPacket& operator << (unsigned int Data)
	{
	...
	inline SFPacket& operator >> (char* szStr)
	{
		strcpy(szStr, (char*)&m_pPacketData[m_usCurrentReadPosition]);
		m_usCurrentReadPosition += strlen((char*)&m_pPacketData[m_usCurrentReadPosition]) + 1;

		return *this;
	}

	void Read(unsigned char* pBuffer, int BufferSize)
	{
	...
	void Write(const unsigned char* pBuffer, int BufferSize)
	{
