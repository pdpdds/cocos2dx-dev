...
float Ch1_AnimatingSprites::makeBatFlyUp(SimpleAnimObject* bat) {
	...
	for(int i=1; i<=4; i+=1){
		animation->addSpriteFrame(cache->spriteFrameByName(CCString::createWithFormat("simple_bat_0%i.png",num)->getCString()));
		num++;
		if(num > 4){ num = 1; }
	}
	...
	bat->stopAllActions();
	bat->runAction(CCRepeatForever::create(CCAnimate::create(animation)));
...
void Ch1_AnimatingSprites::step(float delta) {
...
CCScene* Ch1_AnimatingSprites::scene()
{
    ...
    do 
    {
        ...
        Ch1_AnimatingSprites *layer = Ch1_AnimatingSprites::create();
        CC_BREAK_IF(! layer);
