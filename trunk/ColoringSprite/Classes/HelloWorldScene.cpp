...
bool HelloWorld::init()
{
    ...
    do 
    {
		...
		CCLabelTTF* pLabel = CCLabelTTF::create("Coloring sprites", "Arial", 24);
        CC_BREAK_IF(! pLabel);
...
void HelloWorld::runRecipe() {
	...
	CCSprite* goodSamurai = CCSprite::create("samurai_good.png", CCRectMake(0, 0, winSize.width*0.75f, winSize.height));
	...
	CCSprite* evilSamurai = CCSprite::create("samurai_evil.png", CCRectMake(0, 0, winSize.width*0.75f, winSize.height));
...
void HelloWorld::glowAt(CCPoint pos,CCSize size, ccColor3B color, float rotation, CCSprite* sprite) {
	CCSprite *glowSprite = CCSprite::create("fire.png");
	glowSprite->setColor(color);
	glowSprite->setPosition(pos);
	glowSprite->setRotation(rotation);
	//glowSprite->setBlendFunc(new ccBlendFunc(GL_ONE,GL_ONE));
	glowSprite->runAction(CCRepeatForever::create((CCActionInterval*)CCSequence::create(
...
void HelloWorld::initButtons() {
	...
	CCMenuItemFont *fadeToBlack = CCMenuItemFont::create("FADE TO BLACK");
...
void HelloWorld::fadeToBlackCallback(CCObject* sender) {
	...
	fadeSprite->runAction(CCSequence::create(
