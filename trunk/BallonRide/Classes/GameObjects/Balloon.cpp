...
Balloon::init(cocos2d::CCLayer& layer)
{
	...
	std::string frameNamePrefix = "balloon";
	std::string textureFileName = "balloons.png";
	int frameNumber = 3;
	float delay = 0.5f;
	__speed = 1000.0f;
	...
	schedule(schedule_selector(Balloon::move), (float)0.03f);
}
...
Balloon::move(float dt)
{
	...
	if(__pSprite)
	{
		...
		__pMoveAction = CCMoveTo::create((float)moveDuration, newPosition);
