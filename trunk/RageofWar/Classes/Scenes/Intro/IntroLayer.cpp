...
void IntroLayer::startGamePlay()
{
	...
	CCScene *pScene =PlayerSelectorScene::scene(1);
...
bool IntroLayer::init()
{
	...
	if (CCLayer::init())
	{
		...
		CCSprite *introImage = CCSprite::create("Menus/Intro/bgIntro1.png");
		...
		for (int frameNumber=1; frameNumber < 4; frameNumber++) 
		{
			...
			sprintf(frameName, "Menus/Intro/bgIntro%d.png", frameNumber);
		...
		CCFiniteTimeAction *startGameAction = CCCallFunc::create(this, callfunc_selector(IntroLayer::startGamePlay));
