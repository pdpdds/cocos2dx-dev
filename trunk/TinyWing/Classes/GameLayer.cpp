...
ccColor3B GameLayer::generateDarkColor() {
    ...
    int r, g, b;
    while (true) {
        r = rand()%(maxValue-minValue)+minValue;
        g = rand()%(maxValue-minValue)+minValue;
        b = rand()%(maxValue-minValue)+minValue;
...
void GameLayer::generateBackground() {
    ...
    colors[nVertices++] = ccc4f(1, 1, 1, 0);
    vertices[nVertices] = ccp(textureSize, 0);
    colors[nVertices++] = ccc4f(1, 1, 1, 0);
    vertices[nVertices] = ccp(0, textureSize);
    colors[nVertices++] = ccc4f(1, 1, 1, gradientAlpha);
    ...
    CCSprite *s = CCSprite::create("noise.png");
    ...
    setBackground(CCSprite::createWithTexture(rt->getSprite()->getTexture()));
